...
Dim tab, xls, cs, Conn
  tab= "CORB_Pilote_EDF"
  xls= "C:\Program Files (x86)\JuK\DpuScan\UDD\CORB_Pilote_EDF.xlsx"
...
'MsgBox arRet(1)
  ...
  ' debug de dalil  *******************/ 
  ' MsgBox " vF3Arg = " &  vF3Arg
  ' MsgBox "Le contenu de ExtFunction! 2 -> " + "|code corbeil = "+res(1)+"||*||"
  ' essai de dalil  *******************/ 
...
On Error Resume Next
  ...
  Else
    ...
    sql= "SELECT * FROM ["+table+"$] WHERE LIBELLE_CORBEILLE ='" & IREF_BENEF &"'"
		...
		If IsObject(oRs) Then
		  ...
		  If oRs.Fields.Count > 1 Then
			  If 1=1 Then
				...
				IF Not IsNull(oRs(4).Value) Then res(1)= CStr(oRs(3).Value) ' code corbeil									
				res(2)="" ' code corbeil
...
Dim   vF3Arg 'Global variable for use in EntryPoint

Function ExtFunctionWrapper(vF3Arg,vF3Event)
'$$ DPUSCAN_DECLARATION_BEGIN
'$$ MODE Process
'$$ VAR vF3Arg   = %(D.SMARTGEC_LCORB)
...
Dim res
  ...
  ' debug de dalil  *******************/ 
 '  MsgBox "Le contenu de de la fonction ExtFunctionWrapper 1 vF3Arg = " & vF3Arg & " vF3Event = " & vF3Event
  ' essai de dalil  *******************/ 
